...
#Overall game is to introduce a random sequence with the two buttons and if you are able to repeat the given sequence, the number of inputs increases by 1
#it increases all the way up to 10 steps and you win
#if you fail at matching one of the sequences, you will start over

# Initialize LEDs
led_pins = [12, 13]
leds = [machine.Pin(pin, machine.Pin.OUT) for pin in led_pins] #this for just sets everything in the led pins as an output 

# Initialize buttons
button_pins = [14, 15] 
buttons = [machine.Pin(pin, machine.Pin.IN, machine.Pin.PULL_UP) for pin in button_pins] #and everything in the button_pins array as an input

#Defining function that will flash the given LED number 
...
#This function waits for the button press and then returns the number
def wait_for_button():
    while True:
